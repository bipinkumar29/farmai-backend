const fs = require('fs');
const file = 'D:/AI-Based Farmer Query support and Advisory System 1/src/components/Chatbot.jsx';
let content = fs.readFileSync(file, 'utf8');

const massiveKB = `/* ─── Offline fallback knowledge base ─────────────────────────────────────── */
const OFFLINE_KB = [
...
const insertIndex = lines.findIndex(line => line.includes('/* ─── Sentiment detection ──────────────────────────────────────────────────── */'));

if (insertIndex !== -1) {
  lines.splice(insertIndex, 0, massiveKB);
  fs.writeFileSync(file, lines.join('\\n'));
  console.log('Successfully completed Chatbot replacements.');
} else {
